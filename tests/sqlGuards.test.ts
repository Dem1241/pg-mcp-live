...
describe("validateReadOnlySelectQuery", () => {
  ...
  it("rejects DROP statements", () => {
    expect(() => validateReadOnlySelectQuery("DROP TABLE products")).toThrow(
      "Only SELECT queries and read-only WITH queries are allowed.",
    );
  });

  it("rejects DELETE statements", () => {
    expect(() => validateReadOnlySelectQuery("DELETE FROM products")).toThrow(
      "Only SELECT queries and read-only WITH queries are allowed.",
    );
  });

  it("rejects UPDATE statements", () => {
    expect(() => validateReadOnlySelectQuery("UPDATE products SET name = 'x'")).toThrow(
      "Only SELECT queries and read-only WITH queries are allowed.",
    );
  });
  ...
  it("rejects multiple statements", () => {
    expect(() =>
      validateReadOnlySelectQuery("SELECT * FROM products; DROP TABLE customers;"),
    ).toThrow("Multiple SQL statements are not allowed.");
  });

  it("rejects forbidden keywords inside SELECT queries", () => {
    expect(() =>
      validateReadOnlySelectQuery("SELECT * INTO temp_products FROM products"),
    ).toThrow("Forbidden SQL keyword detected: INTO");
  });

  it("rejects row-locking clauses", () => {
    expect(() =>
      validateReadOnlySelectQuery("SELECT * FROM products FOR UPDATE"),
    ).toThrow("Row-locking clauses are not allowed.");
  });
